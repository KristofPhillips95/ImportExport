...
define_sets_simplified!(m,scenario,year,CY,[],[investment_country])
process_parameters_simplified!(m,scenario,year,CY,[investment_country])
process_time_series!(m,scenario,year,CY_ts, true,endtime)
remove_capacity_country!(m,investment_country,true)
...
total_prod_per_tech = Dict(tech => sum([JuMP.value.(m.ext[:variables][:production][country,tech,t]) for t in 1:endtime]) for tech in m.ext[:sets][:technologies][country] )

bar(collect(keys(total_prod_per_tech)), collect(values(total_prod_per_tech)), orientation=:vertical)
bar(collect(keys(total_prod_per_tech)), collect(values(total_prod_per_tech)), orientation=:vertical)
