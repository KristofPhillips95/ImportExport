...
build_NTC_investment_model!(m,endtime,VOLL,0.1,0.07,simplified)
optimize!(m)
...
total_prod_per_tech = Dict(tech => sum([JuMP.value.(m.ext[:variables][:production][country_plot,tech,t]) for t in 1:endtime]) for tech in m.ext[:sets][:technologies][country_plot] )

#Check the production per tech
